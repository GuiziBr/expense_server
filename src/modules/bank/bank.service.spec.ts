...
describe("BankService", () => {
	...
	describe("getById", () => {
		it("should throw Internal server error exception", async () => {
			vi.spyOn(databaseService.bank, "findUnique").mockRejectedValue(
				new Error()
			)
			...
			expect(loggerSpy).toBeCalledWith(
				"Error - Error - getting bank by id bank-id"
			)
		})
	...
	describe("create", () => {
		it("should throw Internal server error exception", async () => {
			...
			expect(loggerSpy).toBeCalledWith(`Error - Error - creating bank ${name}`)
		})
