...
describe("PaymentTypeService", () => {
	...
	describe("getAll", () => {
		it("should throw Internal server error exception", async () => {
			vi.spyOn(databaseService.paymentType, "findMany").mockRejectedValue(
				new Error()
			)
			...
			expect(loggerSpy).toBeCalledWith(
				"Error - Error - getting all payment types"
			)
		})
	...
	describe("getById", () => {
		it("should throw Internal server error exception", async () => {
			...
			expect(loggerSpy).toBeCalledWith(
				"Error - Error - getting payment type by id payment-id"
			)
		})
	...
	describe("create", () => {
		it("should throw Internal server error exception", async () => {
			vi.spyOn(databaseService.paymentType, "upsert").mockRejectedValue(
				new Error()
			)
			...
			expect(loggerSpy).toBeCalledWith(
				"Error - Error - creating payment type payment"
			)
		})
	...
	describe("update", () => {
		it("should throw payment not found exception", async () => {
			...
			).rejects.toThrow("Payment type not found")
		...
		it("should throw payment type already exists exception", async () => {
			await expect(
				paymentTypeService.update("payment-id", "updated-payment", true)
			).rejects.toThrow("There is already a payment type with same description")
			...
			expect(loggerSpy).toBeCalledWith(
				'Payment type with description "updated-payment" already exists'
			)
		})
